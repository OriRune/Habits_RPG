...
**The Homestead (`town` slice):** A persistent isometric town-builder (not a run) — the repeatable gold + material sink. Rules live in `src/engine/town.ts` (pure reducer: placement, labor pipeline, prestige, perks, refunds); catalogs in `src/content/townBuildings.ts` + `townDecor.ts`; renderer in `src/components/town/` (`iso.ts` projection, `TownCanvas.tsx`, `townArt.tsx` procedural SVG). Placing/upgrading a building charges gold + materials up front, then completes via habit-earned *labor*; completed buildings grant light, non-resource perks (sight, stamina, merchant discount, trial practice, +energy cap, labor discount, +queue slot, Forge sweet-zone) and *prestige*, which gates land deeds (the pure-gold ≥500g sink) and some buildings. `town` is persistent state (persist `version: 34`, `freshTown()` on old saves) and rides the cloud blob (not in `TRANSIENT_KEYS`). Forward-compat: the future read-only party-visit payload is `TownState` verbatim (`v: 1`) — see the freeze doc block in `engine/town.ts`.
