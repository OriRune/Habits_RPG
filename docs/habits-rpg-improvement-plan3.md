...
| # | Item | Findings | Status |
|---|---|---|---|
| 10.1 | **Engine + catalog + slice + persistence (M1)**: pure `engine/town.ts` (placement/occupancy/labor/prestige/refund rules), `content/townBuildings.ts`/`townDecor.ts` catalogs, `townSlice` spend actions on the `craft()` pattern, persist v34 (`town ?? freshTown()`; v33 is the Forge quality tiers), and `stone_lode`/`timber_stand` source nodes in mine/forest content so the dead materials start dropping early. A later v37 migration (2026-07-10) heals orphaned upgrade projects (TOWN-02). | BAL-05, BAL-16, homestead plan | ✅ |
